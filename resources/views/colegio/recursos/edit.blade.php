{{-- filepath: c:\laragon\www\colegio\resources\views\colegio\recursos\edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.recursos.update', $recurso) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="curso_id" id="curso_id" required
                            ...
                            @foreach($cursos as $curso)
                                <option value="{{ $curso->id }}" {{ old('curso_id', $recurso->curso_id) == $curso->id ? 'selected' : '' }}>
                                    {{ $curso->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $recurso->titulo) }}" required maxlength="200"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200">
                        @error('titulo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <select name="tipo" id="tipo" required
                            ...
                            @foreach($tipos as $key => $value)
                                <option value="{{ $key }}" {{ old('tipo', $recurso->tipo) == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="url" id="url" value="{{ old('url', $recurso->url) }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200">
                        @error('url')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="descripcion" id="descripcion" rows="4"
                                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200">{{ old('descripcion', $recurso->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
